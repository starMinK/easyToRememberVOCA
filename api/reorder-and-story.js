...
5) story 생성 기준 (아주 중요):
   - 한국어 발음을 ‘그럴듯하게 비틀어서’ 단어의 의미가 상상되는 단어를 넣어를 만들어라.
   - 발음을 그대로 읽는 한글 단어는 사용을 금지한다.
   - 의미를 단순히 설명하는 문장 또한 금지한다.
   - 단어의 발음 일부를 이용하되, 반드시 ‘영단어가 연상되는’단어를 포함하여 문장을 만들어야 한다.
   - "뜻을 외우자", "꼭 기억해" 같은 메타 문장은 금지한다. 일상속에서 말하는 문장으로 만들어라.
   - 알파벳을 섞거나 영어 스펠링을 스토리 안에 직접 쓰지 말 것.
     단, 영단어가 연상되는 한글 옆에는 괄호로 단어를 알려주는 경우는 허용한다. (예: ~~~(word))
   - 스토리는 "발음 비틀기 + 이미지 + 의미 연결" 3요소가 모두 포함된 한 문장아면 좋다.
   - 말투는 친근하게.
