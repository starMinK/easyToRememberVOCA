...
3) rootword는 존재하면 (xxx)뜻 형식으로 적어라. 무조건 영단어는 어원이 존재한다. 찾아서 꼭 넣어라.
   두가지 이상의 어원이 존재한다면 각 어원마다 (xxx)뜻+(xxx)뜻과 같이 +로 연결하고 각 어원마다 의미를 괄호안에 넣는 이 형태를 따라라.

4) 모든 단어는 반드시 story를 1줄 생성해야 한다.  
   story가 비어 있는 경우("")는 절대 허용하지 않는다.

5) story 생성 기준 (아주 중요):
   - 학생들이 해당 단어를 쉽게 외울 수 있도록 도와주는 역할을 한다.
   - 한국어 발음이 영단어의 영어 발음과 비슷한 한글 단어와 영단어의 한글 뜻을 조합한 문장을 만들어라. (예: (prove -> 프로 v)는 그가 잘난걸 증명해준다.)
   - 영단어의 발음을 그대로 표현한 한글 사용은 금지한다. (예: prove -> 프루브 사용 금지)
   - 의미를 단순히 설명하는 문장 또한 금지한다.
   - "뜻을 외우자", "꼭 기억해" 같은 메타 문장은 금지한다. 
   - 꼭 일상속에서 사용되는 단어 구성으로 문장을 만들어라.
   - 알파벳을 섞거나 영어 스펠링을 스토리 안에 직접 쓰지 마라.
     단, 영단어 발음이 연상되는 한글 옆에는 반드시 괄호로 단어를 알려줘야한다. (예: ~~~(word))
   - 말투는 친근하게 작성해라.
   - 글 문장은 한 줄 이내로 작성해야한다.

6) 모든 단어는 서로 독립적으로 완성된 story를 가져야 한다.
`.trim();
